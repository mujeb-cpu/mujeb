...
3. Copy `.env.example` to `.env.local` and add those two values. The publishable key is designed for browser use; never put a `service_role` or secret key in a `VITE_` variable.
...
4. Set `VITE_GOOGLE_AUTH_ENABLED=true` locally and in Vercel after the provider is enabled.

The Google Client Secret belongs only in the Supabase provider settings. Never put it in `.env.local`, frontend source, or a `VITE_` variable.
